...
import { deliverableAndimpactTracklineHeading, documentsHeadings } from "../constants";
...
const data = [
	{
		fileName: "Screenshot",
		transactionForm: "Deliverable Target",
		size: "150kb",
		type: "pdf",
		date: "12-01-2020",
		link:
	...
	useEffect(() => {
		...
		data?.map((document, index) => {
			let row = [
				...
				<TableCell key={index}>{document.fileName}</TableCell>,
				<TableCell key={index}>{document.transactionForm}</TableCell>,
				<TableCell key={index}>{document.size}</TableCell>,
				<TableCell key={index}>{document.type}</TableCell>,
				<TableCell key={index}>{document.date}</TableCell>,
				<TableCell key={index}>
					<IconButton
						onClick={() => {
							var win = window.open(document.link, "_blank");
							win?.focus();
						}}
					>
						<VisibilityIcon />
